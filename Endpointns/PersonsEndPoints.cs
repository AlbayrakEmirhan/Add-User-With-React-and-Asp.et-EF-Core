...
public static class PersonsEndPoints
{
    ...
    public static RouteGroupBuilder MapPersonsEndPoints(this WebApplication app)
    {
        ...
        //GET USERT /GET
        app.MapGet("persons", async (DirectoryContex dbContex) => await dbContex.people.Select(person => person.ToUsersSummaryDto()).AsNoTracking().ToListAsync());
        ...
        group.MapPost("/", async (CreatePersonDtos newPerson, DirectoryContex dbContex) =>
        {
            ...
            return Results.CreatedAtRoute(GetUserEndPoint, p.ToUsersDetailsDto());
        });
        ...
        group.MapPut("/{id}", async (int id, UpdatePersonDtos updatePerson, DirectoryContex dbContex) =>
        {
            ...
            dbContex.Entry(existPerson).CurrentValues.SetValues(updatePerson.ToEntity(id)); //geçerli nesneleri update p ile değiştiriyorum
            await dbContex.SaveChangesAsync();
